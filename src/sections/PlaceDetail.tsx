  ...
  return (
    <div className="min-h-screen" style={{ background: "var(--bg-primary)" }}>
      ...
      <div className="relative h-72 sm:h-96 lg:h-[28rem]">
        ...
        <button
          onClick={() => navigate(-1)}
          className="absolute top-4 left-4 flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium text-white transition-all hover:scale-105"
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 pb-20">
          ...
          >
            ...
            {place.website && (
              <a
                ...
                className="inline-flex items-center gap-2 mt-4 text-sm transition-opacity hover:opacity-70"
        ...
        <section className="mb-12">
          <h2 className="text-xl sm:text-2xl font-bold mb-4 flex items-center gap-2" style={{ color: "var(--text-primary)", fontFamily: "var(--font-heading)" }}>
            <Wine size={20} style={{ color: "var(--accent)" }} />
            Настойки
          </h2>
          <div
            className="rounded-2xl p-5 sm:p-6"
          ...
          >
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-sm" style={{ fontFamily: "var(--font-body)", opacity: 0.9 }}>
                {place.infusions.highlight}
              </span>
            </div>

            <div className="space-y-3 mb-5">
              {place.infusions.list.map((inf) => (
                <div key={inf.name} className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <span className="text-sm ml-2" style={{ fontFamily: "var(--font-body)", opacity: 0.8 }}>— {inf.note}</span>
                  </div>
                </div>
              ))}
            </div>

            {place.infusions.signature && (
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                ...
                <ul className="space-y-1.5">
                  {place.externalReviews.pros.map((p) => (
                    <li key={p} className="text-sm flex items-start gap-2" style={{ color: "var(--text-secondary)", fontFamily: "var(--font-body)" }}>
                      <span className="w-1.5 h-1.5 rounded-full shrink-0 mt-1.5" style={{ background: "var(--success)" }} />
                      {p}
                    </li>
                  ))}
                </ul>
              ...
              <div>
                ...
                <ul className="space-y-1.5">
                  {place.externalReviews.cons.map((c) => (
                    <li key={c} className="text-sm flex items-start gap-2" style={{ color: "var(--text-secondary)", fontFamily: "var(--font-body)" }}>
                      <span className="w-1.5 h-1.5 rounded-full shrink-0 mt-1.5" style={{ background: "var(--danger)" }} />
                      {c}
                    </li>
                  ))}
                </ul>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <textarea
                  ...
                  placeholder="Поделитесь впечатлениями о настойках..."
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 text-xs" style={{ color: "var(--text-muted)", fontFamily: "var(--font-body)" }}>
                    ...
                    <span>Будьте вежливы. Нецензурная лексика удаляется.</span>
                  </div>
                  <button
                    onClick={handleSubmit}
                    disabled={!newComment.trim()}
                    className="flex items-center gap-2 rounded-lg px-5 py-2 text-sm font-medium text-white transition-all hover:scale-105 disabled:opacity-50"
